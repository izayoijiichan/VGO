...
namespace UniGLTFforUniVgo
{
    ...
    public static class AnimationExporter
    {
        ...
        /// <returns></returns>
        private static int AddChannelAndGetSampler(glTFAnimation gltfAnimation, int nodeIndex, glTFAnimationTarget.AnimationProperties property)
        {
            // find channel
            var channel = gltfAnimation.channels.FirstOrDefault(x => x.target.node == nodeIndex && x.target.path == glTFAnimationTarget.GetPathName(property));
            if (channel != null)
            {
            ...
            var samplerIndex = gltfAnimation.samplers.Count;
            var sampler = new glTFAnimationSampler();
            gltfAnimation.samplers.Add(sampler);
            ...
            gltfAnimation.channels.Add(channel);
